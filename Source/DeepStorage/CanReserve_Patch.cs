...
namespace Share_The_Load.DeepStorage
{
	static class CanReserve_Patch
	{
		...
		{
			/*if (target.HasThing)
				Verse.Log.Message($"{claimant} can reserve {stackCount} of [{target.Label}] {target.Thing.ToStringSafe()} ({target.Thing.stackCount}) @ {target.Cell}?");
			else
				Verse.Log.Message($"{claimant} can reserve {stackCount} of [{target.Label}] {target.Cell}?");*/
