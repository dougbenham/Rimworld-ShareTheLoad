...
namespace Share_The_Load.DeepStorage
{
	static class Reserve_Patch
	{
		...
		{
			if (target.HasThing)
				Log.Message($"{claimant} reserved {stackCount} of [{target.Label}] {target.Thing.ToStringSafe()} ({target.Thing.stackCount}) @ {target.Cell} during job {job.ToStringSafe()}");
			else
				Log.Message($"{claimant} reserved {stackCount} of [{target.Label}] {target.Cell} during job {job.ToStringSafe()}");
